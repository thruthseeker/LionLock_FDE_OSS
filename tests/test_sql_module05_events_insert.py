import json

from lionlock.logging import sql_init
from lionlock.logging import events_sql
from lionlock.logging.privacy import contains_forbidden_keys


def test_module05_events_insert_sqlite_memory() -> None:
    uri = "sqlite:///:memory:"
    ok, message = sql_init.init_schema(uri)
    assert ok, message

    event = {
        "session_id": "session-1",
        "turn_index": 1,
        "timestamp": "2025-01-01T00:00:00Z",
        "signal_bundle": {
            "signal_scores": {"hallucination_risk": 0.2},
            "derived_signals": {"fatigue_risk_index": 0.3},
            "missing_inputs": [],
        },
        "gating_decision": "WARN",
        "decision_risk_score": 0.3,
        "trigger_signal": "fatigue_risk_index",
        "trust_logic_version": "v1",
        "code_fingerprint": "fp",
        "prompt_type": "qa",
        "response_hash": "hash-events-1",
        "replay_id": "replay-1",
        "severity": "yellow",
    }

    ok, message = events_sql.record_gating_event(uri_or_dsn=uri, event=event)
    assert ok, message

    conn = events_sql._memory_connection()
    row = conn.execute(
        "SELECT gating_decision, signal_bundle, replay_id FROM events WHERE response_hash=?",
        ("hash-events-1",),
    ).fetchone()
    assert row is not None
    gating_decision, signal_bundle, replay_id = row
    assert gating_decision == "REFRESH"
    assert replay_id == "replay-1"

    bundle_payload = json.loads(signal_bundle)
    assert not contains_forbidden_keys(bundle_payload)

    columns = [row[1] for row in conn.execute("PRAGMA table_info(events)")]
    for field in sql_init.MANDATORY_FIELDS:
        assert field in columns
    assert "replay_id" in columns


def test_module05_events_rejects_forbidden_value_content() -> None:
    uri = "sqlite:///:memory:"
    ok, message = sql_init.init_schema(uri)
    assert ok, message

    event = {
        "session_id": "session-2",
        "turn_index": 2,
        "timestamp": "2025-01-01T00:00:00Z",
        "signal_bundle": {
            "signal_scores": {"hallucination_risk": 0.2},
            "derived_signals": {"fatigue_risk_index": 0.3},
            "missing_inputs": [],
        },
        "gating_decision": "ALLOW",
        "decision_risk_score": 0.2,
        "trigger_signal": "hallucination_risk",
        "trust_logic_version": "v1",
        "code_fingerprint": "fp",
        "prompt_type": "qa",
        "response_hash": "hash-2",
        "severity": "green",
        "notes": "prompt: do not store this",
    }

    ok, message = events_sql.record_gating_event(uri_or_dsn=uri, event=event)
    assert ok is False
    assert "forbidden content" in message.lower()
