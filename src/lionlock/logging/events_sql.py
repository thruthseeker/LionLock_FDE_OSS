...
def _normalize_severity(value: Any, decision_risk_score: float) -> str:
    ...
    if lowered in ALLOWED_SEVERITIES:
        return lowered
    return severity_band(decision_risk_score)
...
def record_gating_event(
    *,
    ...
    if not isinstance(event, dict):
        return False, "Event payload must be a dict."
    ...
    if not ok:
        return False, message or "Event payload contains forbidden keys"
    payload = cleaned
    found = find_forbidden_content(payload)
    if found:
        return False, f"Event payload contains forbidden content at {found}"
    ...
    signal_bundle_json = _serialize_json(bundle_payload)
    event_type = "gating_decision"
    event_severity = _normalize_severity(payload.get("severity"), decision_risk_score)

    row_data: Dict[str, Any] = {
        "session_id": session_id,
        "turn_index": turn_index,
        "timestamp": timestamp,
        "signal_bundle": signal_bundle_json,
    ...
    validate_identifier("events", "table")
    ...
    if sqlite_path is not None:
        ...
        try:
            ...
            placeholders = ",".join("?" for _ in EVENTS_COLUMNS)
            sql = f"INSERT INTO events ({','.join(EVENTS_COLUMNS)}) VALUES ({placeholders})"
            conn.execute(sql, [row_data[col] for col in EVENTS_COLUMNS])
            conn.commit()
    ...
    try:
        stmt = text(
            (
                f"INSERT INTO {table} "
                f"({','.join(EVENTS_COLUMNS)}) "
                f"VALUES ({','.join(':'+col for col in EVENTS_COLUMNS)})"
            )
        )
